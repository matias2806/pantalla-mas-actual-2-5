...
void printPantallasConPrecioMenorA(Pantalla* lista, int len, int valor){
    int i;

    for(i=0;i<len;i++){
        if(lista[i].isEmpty==0){
            if(lista[i].precio <valor){
                printf("%d %s\n",lista[i].idPantalla, lista[i].nombre);
            }
        }
    }
}
...
void ListadosMenu(){
    ...
    while(opcion!=12)
    {
        getIntInRange(&opcion,"\nA.1 Realizar un solo listado de las pantallas ordenadas por los siguientes criterios:\no Precio (descendentemente)\no Nombre (ascendentemente)\n"
        "B.2.  Todas las pantallas que su valor es menor o igual a 10.\n"
        "C.3  Todas las pantallas que su valor es mayor a 10.\n"
        "D.4  Todas las pantallas que superan el promedio de los precios.\n"
        "E.5  Todas las pantallas que no superan el promedio de los precios.\n"
        "F.6  Todas las contrataciones cuya cantidad de dias es menor o igual a 10,indicando nombre de la pantalla contratada, precio unitario, dias y precio total de la contratacion\n"
        "G.7  Listado de clientes en el cual se indique la cantidad de contrataciones que ha realizado\ny por que importe. Ordenado por los siguientes criterios:\no Importe (descendentemente)\no Cuit (ascendentemente)\n"
        "H.8  La o las pantallas con mas contrataciones.\n"
        "I.9  La o las pantallas con mas clientes. \n"
        "J.10  La o las pantallas que mas ha facturado.\n"
        "K.11  La o las pantallas que han facturado\n"
        ,"ERROR\n",1,12,3);
        switch(opcion)
        {
            case 1:
